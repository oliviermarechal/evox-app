...
export default function SlideToAction({
  ...
  const containerWidth = Math.max(width, 300);
  const containerHeight = Math.max(height, 70);
  const borderRadius = containerHeight / 2;
  const knobSize = orientation === 'horizontal' ? containerHeight - 2 : containerHeight - 2;
  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd(() => {
      if (orientation === 'horizontal') {
        const threshold = (containerWidth - knobSize) * 0.7;
        ...
        const threshold = (containerHeight - knobSize) * 0.7;
  ...
  const progressStyle = useAnimatedStyle(() => ({
    ...(orientation === 'horizontal' 
      ? { 
          width: knobX.value + (knobSize  ),
          height: containerHeight,
          left: -1,
          top: -1
        }
      : { 
          height: knobY.value + (knobSize  ),
  ...
  return (
    <View style={[
      ...
      <Animated.View style={[styles.progress, progressStyle, { borderRadius: borderRadius }]} />
      <GestureDetector gesture={panGesture}>
        <Animated.View style={[
          styles.knob, 
          knobStyle,
          orientation === 'vertical' ? [styles.knobVertical, { marginLeft: -knobSize / 2 }] : {},
          {
            width: knobSize,
            height: knobSize,
          ...
          <LinearGradient
            ...
            style={[styles.knobGradient, { borderRadius: (knobSize - 2) / 2 }]}
          />
        </Animated.View>
      </GestureDetector>
      <Animated.Text style={[
