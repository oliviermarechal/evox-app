...
interface EMOMFinalScreenProps {
  finalTime: string;
  currentRound: number;
  durationPerRound: number; // Duration per round in seconds
  onReset: () => void;
  isLandscape?: boolean;
}

export default function EMOMFinalScreen({ 
  finalTime, 
  currentRound, 
  durationPerRound,
  onReset, 
  isLandscape = false 
}: EMOMFinalScreenProps) {
  // Animation values
        ...
        <Animated.View style={[containerStyle, finalAnimationStyle]}>
          ...
          <View style={{
            ...
            <View style={{
              ...
              <View style={{ alignItems: 'center', flex: 1 }}>
                <Text style={numberStyle}>
                  {Math.floor(durationPerRound / 60)}:{(durationPerRound % 60).toString().padStart(2, '0')}
                </Text>
                <Text style={labelStyle}>
                  PER ROUND
                </Text>
